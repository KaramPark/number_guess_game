...
const NumberGuessGame = () => {
    ...
    <div className="flex flex-col items-center gap-8 p-8 bg-white shadow-xl rounded-xl max-w-md">
        <h1 className="text-2cl font-bold">
            숫자 맞추기 게임
        </h1>
         ...
         type="number" value={guess} onChange={(e) => setGuess(e.target.value)}  disabled={isWin}/>
         <button
            className="bg-blue-400 text-white px-6 py-2 rounded-lg hover:bg-blue-500 disabled:opacity-50"
            onClick={handleSubmit}
            disabled={isWin}>확인</button>
        <p className="text=lg font-medium"> {message}</p>

        <div className="w-full">
            <h2 className="font-semibold mb-2">입력 기록</h2>
            <ul className="list-disc list-inside text-sm text-gray-600">
                {history.map((num,index) => (<li key={index}> {num} </li>))}</ul>

        </div>

        {isWin && <button className="mt-4 px-5 py-2 border border-gray-400 rounded-lg hover:bg-gray-100" onClick={handleRestart}> 다시시작 </button>}
    </div>
    );
};
